...
Exit_Bulk_04_01_02_Verify that all patches on Bulk patching list are patched successfully after selecting Exit button if patching on a 48-port Legacy Copper
    ...
    Then Check if Rack "1" Panel "2" Row "1" from Port "1" "appeared" in Trace screen
    Then Check if Rack "1" Panel "2" Row "1" from Port "4" "appeared" in Trace screen
    Then Check if the "full" of "Copper" Patching Bar "appeared" in Trace screen
    
    And Press Copper port at Rack "1" Panel "2" Row "1" Port "2" with keymode "short"
    Then Check if Rack "1" Panel "2" Row "1" from Port "2" "appeared" in Trace screen
    Then Check if Rack "1" Panel "2" Row "1" from Port "5" "appeared" in Trace screen
    Then Check if the "full" of "Copper" Patching Bar "appeared" in Trace screen
    
    And Press Copper port at Rack "1" Panel "2" Row "1" Port "3" with keymode "short"
    Then Check if Rack "1" Panel "2" Row "1" from Port "3" "appeared" in Trace screen
    Then Check if Rack "1" Panel "2" Row "1" from Port "6" "appeared" in Trace screen
    Then Check if the "full" of "Copper" Patching Bar "appeared" in Trace screen
